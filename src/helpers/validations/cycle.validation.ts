...
export const cycleValidation = Joi.object().keys({
    ...
    arrival_date:Joi.date().format("YYYY-MM-DD").required().ruleset.greater(Joi.ref('departure_date'))
    .rule({ message: 'arrival_date must be greater than departure_date' }),

    return_date:Joi.date().format("YYYY-MM-DD").required().ruleset.greater(Joi.ref('arrival_date'))
    .rule({ message: 'arrival_date must be greater than departure_date' }),

    return_arrival_date:Joi.date().format("YYYY-MM-DD").required().ruleset.greater(Joi.ref('departure_date'))
