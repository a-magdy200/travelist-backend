...
const showHotel = async (req: Request, res: Response) => {
  ...
  const hotel: Hotel | null = await AppDataSource.manager.findOneBy<Hotel>(Hotel, {
    id
  });
  if (hotel) {
    res.json({
...
const updateHotel = async (req: Request, res: Response) => {
  try {
    const id: number | undefined = +req.params.id;
    const validation: Hotel = await hotelValidation.validateAsync(req.body, { abortEarly: false });
    const updateResult = await AppDataSource.manager.update<Hotel>(Hotel, {
      id
    }, validation);
